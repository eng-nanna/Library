 ...
 <input type="submit" class="button" value="Search" name="submit">
      ...
      <table width="100%">
        ...
        <?php
		  ...
		  $query="SELECT * FROM users LIMIT $start_from, $num_rec_per_page";
		  $result= mysqli_query($db_conn, $query) or die("Invalid query1");
		  while($row = mysqli_fetch_array($result)){
			  $query1="SELECT * FROM borrows WHERE username = '$row[username]'";
			  $res1= mysqli_query($db_conn, $query1) or die("Invalid query2");
			  $count1=mysqli_num_rows($res1);
			  $query2="SELECT * FROM selling WHERE username = '$row[username]'";
			  $res2= mysqli_query($db_conn, $query2) or die("Invalid query3");
