 ...
 <input type="submit" class="button" value="Search" name="submit">
      ...
      <table width="100%">
        <thead>
          ...
          <th class="text-center">No of sold books</th>
        </thead>
        <tbody>
         <?php
		  
			  $query="SELECT username,SUM(copies) AS client_books FROM selling GROUP BY username";
			  $result= mysqli_query($db_conn, $query) or die("Invalid query");
			  while($row = mysqli_fetch_array($result)){
		  ?>
          <tr class="text-center">
            <td><a href="s_details.php?user=<?php echo $row['username'];?>"><?php echo $row['username'];?></a></td>
            <td><?php echo $row['client_books']; ?></td>
          </tr>
          <?php } ?>
        </tbody>
      </table>
